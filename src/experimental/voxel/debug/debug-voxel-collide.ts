...
function runHitCubeSpeed() {
  ...
  for (let i = 0; i < 1e7; i++) {
    ...
    nextHitCubeIn(origin, vector, 1, out3); // 2807 => 150 own
...
async function testHitCubeIn() {
  ...
  // XY face (front)
  await assert(() => arrayEquals(nextHitCubeIn([0.5, 0.5, -1] /* on center */, [0, 0, 1], 1, hitPoint), [0.5, 0.5, 0])); // on center
  await assert(() => arrayEquals(nextHitCubeIn([0, 0.5, -1] /* on x = 0 edge */, [0, 0, 1], 1, hitPoint), [0, 0.5, 0])); // on included edge
  await assert(() => !isValidHitPoint(nextHitCubeIn([1, 0.5, -1] /* on x = 1 edge */, [0, 0, 1], 1, hitPoint))); // on excluded edge

  // XY face (back)
  await assert(() => arrayEquals(nextHitCubeIn([0.5, 0.5, 2] /* on center */, [0, 0, -1], 1, hitPoint), [0.5, 0.5, 1])); // on center
  await assert(() => arrayEquals(nextHitCubeIn([0, 0.5, 2] /* on x = 0 edge */, [0, 0, -1], 1, hitPoint), [0, 0.5, 1])); // on included edge
  await assert(() => !isValidHitPoint(nextHitCubeIn([1, 0.5, 2] /* on x = 1 edge */, [0, 0, -1], 1, hitPoint))); // on excluded edge


  // XZ face (front)
  await assert(() => arrayEquals(nextHitCubeIn([0.5, -1, 0.5], [0, 1, 0], 1, hitPoint), [0.5, 0, 0.5])); // on center
}
